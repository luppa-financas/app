...
export function EditTransactionModal({ hook, onSave, onBulkSave }: EditTransactionModalProps) {
  const {
    transaction, category, subcategory, nickname, status,
    matchCount, applyToAll, bulkConfirmOpen,
    close, setCategory, setSubcategory, setNickname, setApplyToAll,
    save, requestBulk, confirmBulk, cancelBulk,
  } = hook;
    ...
    <>
      <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
        ...
        <div className="relative bg-white w-full sm:w-[440px] rounded-t-2xl sm:rounded-2xl shadow-xl p-6 flex flex-col gap-5">
          ...
          <p className="text-xs text-slate-400 -mt-2 truncate">{transaction.alias ?? transaction.description}</p>
          ...
          <div className="flex flex-col gap-3">
            ...
            <div>
              ...
              <input
                ...
                className="w-full bg-white border border-slate-200 rounded-lg px-3 h-9 text-sm text-slate-700 outline-none focus:ring-2 focus:ring-indigo-300 focus:border-indigo-400 placeholder-slate-300 disabled:opacity-50 disabled:cursor-not-allowed"
              />
              {applyToAll && (
                <p className="text-xs text-slate-400 mt-1">Apelido não se aplica à edição em massa</p>
              )}
            </div>
            ...
            {matchCount !== null && matchCount > 1 && (
              <label className="flex items-start gap-2 cursor-pointer pt-1">
                ...
                <span className="text-sm text-slate-700">
                  Aplicar a todas as{' '}
                  <span className="font-medium">{matchCount}</span> transações com{' '}
      ...
      {bulkConfirmOpen && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center">
          <div className="absolute inset-0 bg-black/50" onClick={cancelBulk} />
          <div className="relative bg-white w-full max-w-sm mx-4 rounded-2xl shadow-xl p-6 flex flex-col gap-4">
            <h3 className="text-sm font-semibold text-slate-800">
              Atualizar {matchCount} transações?
            </h3>
            <p className="text-sm text-slate-600">
              Todas as transações com{' '}
              <span className="font-medium">&quot;{transaction.description}&quot;</span> serão
              categorizadas como{' '}
            ...
            {status === 'error' && (
              <p className="text-xs text-red-500">Não foi possível aplicar. Tente novamente.</p>
            )}
            <div className="flex gap-2">
              <button
                onClick={cancelBulk}
                disabled={status === 'saving'}
                className="flex-1 h-9 rounded-lg border border-slate-200 text-sm text-slate-600 hover:bg-slate-50 transition-colors disabled:opacity-60"
              >
                Cancelar
              </button>
              <button
                onClick={() => void confirmBulk(onBulkSave)}
                disabled={status === 'saving'}
                className="flex-1 h-9 rounded-lg bg-indigo-600 text-sm text-white font-medium hover:bg-indigo-700 transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
              >
                {status === 'saving' ? 'Aplicando…' : 'Confirmar'}
              </button>
            </div>
          </div>
        </div>
